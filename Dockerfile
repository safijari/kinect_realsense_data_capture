...
RUN apt-key adv --keyserver keys.gnupg.net --recv-key F6E65AC044F831AC80A06380C8B3A55A6F3EFCDE || apt-key adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-key C8B3A55A6F3EFCDE
...
RUN git clone https://github.com/OpenKinect/libfreenect2.git && cd libfreenect2 && cd /libfreenect2 && mkdir build && cd build && cmake .. -DCMAKE_INSTALL_PREFIX=$HOME/freenect2 -DENABLE_OPENGL=FALSE && make && make install
...
RUN pip3 install numpy imageio==2.9.0
